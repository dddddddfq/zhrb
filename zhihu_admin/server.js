...
app.use(async (req, res, next) => {
	// 不在校验范围内的接口直接下一步
	const checkList = ['/user_info', '/check_login', '/store', '/store_list', '/store_remove',
		'/user_update'
	];
...
app.get('/store_list', async (req, res) => {
	...
	store[Symbol.asyncIterator] = function asyncIterator() {
		...
		return {
			next() {
				if (index > self.length - 1) return Promise.resolve({
					done: true
				});
...
app.post('/phone_code', async (req, res) => {
	...
	// 发送短信
	await fs.appendFile(`${path.resolve(__dirname, '')}/code.txt`,
		`手机号 ${phone} 的用户于 ${new Date(time).toLocaleString('zh-CN', { hour12: false })} 发送短信验证码：${code} \n`,
		'utf-8');
	responsePublic(res, true);
});
...
app.post('/login', async (req, res) => {
	...
	let flag = req.$CODEDATA.find(item => {
		return item.phone === phone && item.code === String(code) && (+new Date() - item.time) <=
			1800000;
	});
	if (!flag) {
